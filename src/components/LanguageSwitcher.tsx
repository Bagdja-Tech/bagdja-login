...
export function LanguageSwitcher() {
  ...
  const changeLanguage = (lang: Language) => {
    ...
    // Preserve redirect_url if exists
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-1 px-2 py-2 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-100 hover:text-slate-900 transition border border-gray-300 bg-white"
      >
        ...
        <div className="absolute right-0 mt-2 w-auto bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50">
          {languageOptions.map((option) => (
            ...
            >
              ...
              {currentLang === option.code && (
                <svg
                  className="w-4 h-4 ml-2 text-slate-700"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path
